...
+ (id)sharedLocationHandler {
    ...
    dispatch_once(&pred, ^{
        locationHandlerSingleton = [[TTLocationHandler alloc] init];
    });
...
- (id) init
{
  if (self = [super init])
  {
      ...
      //borrar regiones por si existen en cache
      
      NSArray *regionArray = [[self.locationManager monitoredRegions] allObjects]; // the all objects is the key
...
- (BOOL)registerNotificationForLocation:(CLLocation *)myLocation withRadius:(NSNumber *)myRadius assignIdentifier:(NSString *)identifier {
    ...
    */
    ...
    NSLog(@"Registered Region");
    return YES;
}


/**
 * Registrar una region para ser monitoreada
 */
- (BOOL) registerRegion:(CLLocationCoordinate2D *)theCenter  withRadius:(NSNumber *)radius assignIdentifier:(NSString *)theIdentifier
{
   CLLocationDistance theRadius = [radius doubleValue];
    CLRegion* region = [[CLRegion alloc] initCircularRegionWithCenter:*theCenter radius:theRadius identifier:theIdentifier];
    
    [self.locationManager startMonitoringForRegion:region
                                   desiredAccuracy:kCLLocationAccuracyNearestTenMeters];
    NSLog(@"Registered Region success");
    return YES;
}
...
- (void)removeAllMonitoredRegions {
    ...
    NSLog(@"Removed all Monitored Regions");
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation
{
    ...
    NSLog(@" Aqui vamos a intentar enviar el dato al server");
    
    ///
    
    //envio datos al api rest de bacab
    
    NSString * latitud1 = [NSString stringWithFormat:@"\"latitud\":\"1234\""];
    NSString * longitud1 = [NSString stringWithFormat:@"\"longitud\":\"1234\""];
    NSString * idmovil1 = [NSString stringWithFormat:@"\"idmovil\":\"heel\""];
    
    
    NSMutableData *responseData1 ;
    
    //Se realiza la peticion
    NSMutableURLRequest *request1 = [NSMutableURLRequest
                                     requestWithURL:[NSURL URLWithString:@"http://apibacab.mocionsoft.com/actalizarPosicionMovil"]];
    
    //organizando el cuerpo de la peticion como un json
    
    NSString *params1 = [[NSString alloc] initWithFormat:@"{%@,%@,%@}",latitud1,longitud1,idmovil1];
    [request1 setHTTPMethod:@"POST"];
    [request1 setHTTPBody:[params1 dataUsingEncoding:NSUTF8StringEncoding]];
...
- (void)locationManager:(CLLocationManager *)manager didEnterRegion:(CLRegion *)region
{
    NSLog(@"entro a region %@",[region identifier]);
...
- (void)locationManager:(CLLocationManager *)manager didExitRegion:(CLRegion *)region
{ NSLog(@"salio a region %@",[region identifier]);}

- (void)locationManager:(CLLocationManager *)manager monitoringDidFailForRegion:(CLRegion *)region withError:(NSError *)error
{
 
    
    NSLog(@"error analizando region: %@", error.description);
